...
final tasksRepoProvider = Provider((ref) => TasksRepository(ref.watch(apiClientProvider)));
...
class TasksRepository {
  ...
  Future<List<Task>> list() async {
    ...
    return ((j['tasks'] as List?) ?? []).map((e) => Task.fromJson(e as Map<String, dynamic>)).toList();
  }

  Future<Task> create({required String title, String? sourceText, String? priority}) async {
  ...
  Future<List<Microtask>> generateMicrotasks(String taskId, {required String goal, int? sessionLength, bool extraSimplify = false}) async {
    ...
    return ((j['microtasks'] as List?) ?? []).map((e) => Microtask.fromJson(e as Map<String, dynamic>)).toList();
  }

  Future<Microtask> updateMicrotask(String id, {String? status, String? label, int? position}) async {
